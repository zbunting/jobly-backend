...
import {
  ...
  commonAfterEach,
  commonBeforeAll,
  commonBeforeEach, testJobIds, u1Token,
} from "./_testCommon.js";
...
describe("DELETE /jobs/:id", function () {
  ...
  test("not found for no such job - admin", async function () {
    const resp = await request(app)
        .delete(`/jobs/0`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  ...
  test("unauth for no such job - non-admin", async function () {
    const resp = await request(app)
        .delete(`/jobs/0`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
  ...
  test("unauth for no such job - anon", async function () {
